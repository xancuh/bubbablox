...
public class PrivateMessagesService : ServiceBase, IService
{
	public async Task<int> CountUnreadMessages(long userId)
	{
		var result = await db.QuerySingleOrDefaultAsync<Total>(
			"SELECT COUNT(*) as total FROM user_message " +
			"WHERE (NOT is_read OR is_read IS NULL) " +
			"AND user_id_to = :user_id " +
			"AND NOT (user_id_from = 1 AND created_at < :year_2022)", 
			new
			{
				user_id = userId,
				year_2022 = new DateTime(2022, 1, 1, 0, 0, 0, DateTimeKind.Utc),
			});
		return result.total;
	}
