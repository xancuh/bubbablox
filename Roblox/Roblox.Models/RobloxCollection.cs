...
public class RobloxCollectionPaginated<T> : RobloxCollection<T>
{
    ...
    public RobloxCollectionPaginated(int limit, int offset, IEnumerable<T> data)
    {
        ...
        nextPageCursor = dataArray.Length >= limit ? (offset + limit).ToString() : null;
        previousPageCursor = (offset - limit) >= 0 ? (offset - limit).ToString() : null;
        this.data = dataArray;
    }

    public RobloxCollectionPaginated(int limit, int offset, IEnumerable<T> data, long totalRap) 
        : this(limit, offset, data)
    {
        this.totalRap = totalRap;
    }
}