...
const UrlUtilities = (() => {
  const getDomainFromUrl = (url) => {
    ...
    if (baseDomainParsed.type === ParseResultType.Listed) {
      ...
      if (baseDomainParsed.hostname === 'zawg.ca') {
        return 'zawg.ca';
      }
      throw new Error('The only allowed reserved domain type is zawg.ca, got ' + baseDomainParsed.hostname);
    } else {
    ...
    for (const item of Object.getOwnPropertyNames(result.headers)) {
      ...
      if (item === 'set-cookie') {
        ...
        if (typeof value === 'string') {
          value = value.replace(/roblox\.com/g, 'zawg.ca');
        } else {
          value.forEach((v, i, arr) => {
            arr[i] = v.replace(/roblox\.com/g, 'zawg.ca');
          });
        }
      }
